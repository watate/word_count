...
int regexWordCount(String text) {
  ...
  return processedText.split(RegExp(r'\s+')).where((word) => word.trim().isNotEmpty).length;
}

// Simple split-based word counter (naive approach)
int splitWordCount(String text) {
  return text.split(RegExp(r'\s+')).where((word) => word.trim().isNotEmpty).length;
}
...
void main() {
  ...
  final words = <String>[];
  final baseWords = ['hello', 'world', 'test', 'sample', 'data', 'word', 'text', 'example'];
  ...
  print('Word counts - New: $newCount, Old: $oldCount, Regex: $regexCount, Split: $splitCount, StringStats: $stringStatsCount');
  ...
  print('Speed: ${(newWordsPerSec / 1000000).toStringAsFixed(1)} million words/s');
  ...
  print('Speed: ${(oldWordsPerSec / 1000000).toStringAsFixed(1)} million words/s');
  ...
  print('Speed: ${(regexWordsPerSec / 1000000).toStringAsFixed(1)} million words/s');
  ...
  print('Speed: ${(splitWordsPerSec / 1000000).toStringAsFixed(1)} million words/s');
  ...
  final stringStatsWordsPerSec = (stringStatsCount * iterations * 1000000) / stringStatsTime;
  final stringStatsCharsPerSec = (text.length * iterations * 1000000) / stringStatsTime;
  ...
  print('Speed: ${(stringStatsWordsPerSec / 1000000).toStringAsFixed(1)} million words/s');
  ...
  print('|--------|---------|------|----------------|');
  print('| New Implementation (Bitmap) | ${(newWordsPerSec / 1000000).toStringAsFixed(1)}M | ${newGBPerSec.toStringAsFixed(3)} | 1.0x |');
  print('| Old Implementation (RegExp) | ${(oldWordsPerSec / 1000000).toStringAsFixed(1)}M | ${oldGBPerSec.toStringAsFixed(3)} | ${(oldWordsPerSec / newWordsPerSec).toStringAsFixed(2)}x |');
  print('| Regex | ${(regexWordsPerSec / 1000000).toStringAsFixed(1)}M | ${regexGBPerSec.toStringAsFixed(3)} | ${(regexWordsPerSec / newWordsPerSec).toStringAsFixed(2)}x |');
  print('| Split | ${(splitWordsPerSec / 1000000).toStringAsFixed(1)}M | ${splitGBPerSec.toStringAsFixed(3)} | ${(splitWordsPerSec / newWordsPerSec).toStringAsFixed(2)}x |');
  print('| StringStats | ${(stringStatsWordsPerSec / 1000000).toStringAsFixed(1)}M | ${stringStatsGBPerSec.toStringAsFixed(3)} | ${(stringStatsWordsPerSec / newWordsPerSec).toStringAsFixed(2)}x |');
  
  print('\n📊 Summary:');
  print('Optimization results for our bitmap-based approach:');
  print('- ${newVsOld.toStringAsFixed(1)}x performance improvement over original implementation');
